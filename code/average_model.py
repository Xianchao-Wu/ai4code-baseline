...
def main():
    ...
    epoch2ckpt = dict()
    if args.val_best:
        checkpoints = glob.glob('{}/model*.bin'.format(args.src_path))
        for ckpt in checkpoints:
            cols = ckpt.split('_')
            epoch = int(cols[1])
            ktau = float(cols[2][4:-4])
            if epoch >= args.min_epoch and epoch <= args.max_epoch:
                val_scores += [[epoch, ktau]]
                epoch2ckpt[epoch] = ckpt
        ...
        #]
        path_list = [
            epoch2ckpt[epoch] for epoch in sorted_val_scores[:args.num, 0]
        ]
    else:
        path_list = glob.glob('{}/[!avg][!final]*.bin'.format(args.src_path))
